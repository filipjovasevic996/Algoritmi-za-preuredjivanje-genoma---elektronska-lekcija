  ...
  return (
    <>
      ...
      <p>
        ...
        <sub>1</sub> ... p<sub>n</sub>) kaže da formiraju susednost ako važi da
        je p<sub>i+1</sub> - p<sub>i</sub> = 1. Po definiciji, za bilo koji
        pozitivan ceo broj k &lt; n, susednosti su (k k + 1), isto kao i (-(k +
        1) k). Ako p<sub>i+1</sub> - p<sub>i</sub> nije jednako 1, onda je (p
        <sub>i</sub> p<sub>i+1</sub>) tačka prekida. Jednostavnije rečeno, tačka
        prekida nastaje između parova uzastopnih elemenata koji su „van reda“ u
        poređenju sa permutacijom identiteta (+1 +2 ... +n). Na primer, parovi
        (+5 -12), (-12 -8), (-6 +1), (+2 +10), (+9 -11) i (-11 +13) su očigledno
        tačke prekida jer nisu susedni. Ali, i par (+10 +9) je takođe tačka
        prekida, uprkos tome što sadrži uzastopne brojeve. Razlog je to što su
        njegovi predznaci van reda u poređenju sa permutacijom identiteta i jer
        je p<sub>i+1</sub> - p<sub>i</sub> = -1. Dodavanjem elemenata na početak
        (nula) i kraj permutacije (n+1), dobija se permutacija (0 p<sub>1</sub>{" "}
        ... p<sub>n</sub> (n+1)), koja ima n+1 parova uzastopnih elemenata: (0 p
